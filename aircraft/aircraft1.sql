...
insert into flights values(106,'Madison','New York',3400,TIMESTAMP '2005-05-13 07:15:31',TIMESTAMP  '2005-05-13 017:15:31',7000);
select * from flights;


-- i Find the names of aircraft such that all pilots certified to operate them have salaries more than Rs.80,000. 
select distinct(a.aname) from aircraft a where a.aid in (select c.aid from certified c where c.eid in (select e.eid from employees e where e.salary>80000));
...
select c.eid,a.cruisingrange from certified c,aircraft a where c.aid =a.aid and (select count(ca.aid) from certified ca where ca.eid=c.eid)>3 order by a.cruisingrange desc limit 1;


-- iii.Find the names of pilots whose salary is less than the price of the cheapest route from Bengaluru to Frankfurt.

select em.ename from employees em where em.eid in( select e.eid from employees e where e.salary<(select min(f.price) from flights f where f.pfrom='Bangalore' and f.pto='Frankfurt'));
...
select  c.aid,avg(e.salary) from employees e,certified c,aircraft a  where e.eid=c.eid and a.cruisingrange>1000 and c.aid=a.aid group by c.aid;



-- v.Find the names of pilots certified for some Boeing aircraft.

select distinct(e.ename) from employees e where e.eid in (select c.eid from certified c where c.eid=e.eid and c.aid in (select a.aid from aircraft a where a.aname='Boeing'));


-- vi. Find the aids of all aircraft that can be used on routes from Bengaluru to New Delhi. 

select a.aid from aircraft a where a.cruisingrange >= (select f.distance from flights f where pfrom='Bangalore' and f.pto='Delhi');
